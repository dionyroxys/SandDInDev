...
public class TileEntityEnhancedFurnace extends TileEntity implements ITickable, IInventory
{
	//can have up too 4 stacks globally in the furnace
	private ItemStackHandler handler = new ItemStackHandler(4);
	private String customName;
	//is it smelting? if so dont restart till its done
	private ItemStack smelting = ItemStack.EMPTY;
	//list for inner mod dependency
...
//
	...
	public static boolean isBurning(TileEntityEnhancedFurnace te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			block_enhanced_furnace.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = EnhancedFurnaceRecipes.getInstance().getAlloyResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = EnhancedFurnaceRecipes.getInstance().getAlloyResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
